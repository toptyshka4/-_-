...
(function(global){
  ...
  function resolveAssetByType(type, el){
    let path = _state.typeToAsset[type] || null;
    if (!path) return null;
    // detect flip/inversion on element or parent container
    let node = el;
    let inverted = false;
    while (node && node !== document.body){
      const ds = node.dataset || {};
      if (ds.side === 'invert' || ds.flip === '1' || ds.inverted === '1' || ds.workingSide === 'right') {
        inverted = true; break;
      }
  ...
  function enhance(root){
    ...
    try{
      // If element does not already have an <img>, add one based on type
      const img = el.querySelector('img') || document.createElement('img');
      if (!img.parentElement) { img.className='wagon-img'; el.appendChild(img); }
      const rec = extractKey(el) && getRecByKey(extractKey(el));
      let typeName = rec && (rec['Модель вагона'] || rec['Тип вагона'] || rec.type);
      if (typeof typeName === 'string'){
        typeName = typeName.toLowerCase();
      }
      const src = resolveAssetByType(typeName, el);
      if (src) img.src = src;
    }catch(e){ log('image resolve failed', e); }
  }
